...
class Preprocess(BaseSetup, Checker, DatabaseHandler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Reprocessing an ingredient that was merely superseded by a closer tolerant_search
        # match is a policy, not a correctness fix, and its scale is not predictable from the
        # database -- it depends on what the search would pick today. Off unless asked for.
        # The fallback exists only for configs written before the key and MUST equal the value
        # in ref/preproc_base.yml -- a fallback that disagrees is a silent override.
        ...
        try:
            ...
            if self.overwrite:
                ...
                self.logger.info(
                    ...
                    "IMAGEID moved are rebuilt; run with dry_run=True first to size it"
                )
    ...
    def load_masterframe(self, device_id=None, use_gpu: bool = True, dry_run: bool = False):
        ...
        # The flatdark trio are plain instance attributes set by the dark branch, so without
        # this they survive into a group whose dark branch never runs -- and that group's flat
        # would then be built from, and stamped with, the previous group's dark. Resetting also
        # keeps `getattr(self, "_flatdark_ppflag", 0)` honest: __getattr__ answers None for an
        # unset name, so the default never applied.
        self.flatdark_output = None
        self.dark_exptime = None
        self._flatdark_ppflag = 0
        # One group's frames share a handful of masters; cleared here because a master
        # regenerated by this group must not be answered for from the previous group's read.
        ...
        for dtype in self.calib_types:
            ...
            generated_now = False
            # _master_ingredients_changed reads headers and logs, so it goes last: the two
            # cheap clauses already answer the common cases.
            if (
                input_file
            ...
            # Once the dark is settled -- generated, fetched or substituted -- the flat's dark
            # term is resolved by one search, the same one in every case. Only when this group
            # actually has a flat to build: the search raises when nothing is found, and a
            # masters-only sweep that stops at dark must not fail on a term it never uses.
    ...
    def _ingredient_change(self, product, selected):
        """Shared wrapper: why `product` is out of date, or None. See prep_utils."""
        return prep_utils.ingredient_change(
            product,
            selected,
    ...
    def _master_change(self, dtype, output_file):
        ...
        Only dark and flat have master ingredients; a bias is combined from raw frames alone,
        which never change. Which masters those are follows `get_header`: a dark records the
        master bias, a flat records the master bias and the flatdark -- which is a separate
        search from `dark_output` and can be another exptime on another night.
        """
        if dtype == "dark":
    ...
    def _sci_change(self, output_file):
        """Why a science output must be (re)produced, or None: missing, or its masters moved."""
    ...
    def _generate_masterframe(self, dtype, device_id, dry_run: bool = False):
        ...
        with acquire_available_gpu(device_id=device_id) as device_id:
            ...
            if dtype == "bias":
                ...
                # flatdark is resolved by _resolve_flatdark once the dark is settled, never
                # from dark_output: the flat wants the shortest dark, not this group's.
    ...
    def _assess_masterframe_quality_and_update_header(
        ...
        # Mint after the merge, never before: the sidecar is carried over from the version
        # being replaced, so merging it back would restore that version's IMAGEID and the
        # regenerated master would silently keep its old identity.
        ...
        # Consumers stamp IMCID from get_image_id, which reads the sidecar first, so the new
        # identity has to reach the sidecar too or every consumer records the old one.
    ...
    def _resolve_flatdark(self, template):
        """Pick the master dark subtracted when combining the master flat.

        Always the minimum-exptime dark on the closest date, whether or not this group built
        a dark of its own. It used to be `dark_output` when the dark was generated here and
        this search only when it was fetched, so two otherwise identical groups got different
        dark terms -- one scaled down from the science exposure, the other from the shortest
        dark available. A flat is a short exposure, so the least-scaled dark is the right one.

        `flatdark_output` is named `_output` for consistency but is never written to disk; the
        only record of which dark a flat actually used is the flat's own IMCMB002/IMCID002.
        """
        max_offset = self.config_node.preprocess.max_offset
    ...
    def data_reduction(self, device_id=None, use_gpu: bool = True, dry_run: bool = False):
        ...
        # Checked per frame: regenerating one master must reprocess the frames that used it,
        # not invalidate the whole preprocess config.
        ...
        self.logger.info(
            ...
            + ", ".join(
                ...
                for kind in ("missing", "regenerated", "sanity", "better-match")
            )
    ...
    def prepare_header(self, dry_run: bool = False):
        ...
        if self.overwrite:
            ...
            # Same predicate as data_reduction, and it must stay that way: this writes the
            # .header sidecar that data_reduction turns into the FITS, so a frame staged
            # here and skipped there would advertise masters its pixels never saw.
