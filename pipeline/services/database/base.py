    ...
    def create_data(self, file, overwrite: bool = False):
        try:
            ...
            if existing_id:
                ...
                # Insert new record
                # Filter out None values and auto-generated fields
                # - id: always excluded (auto-generated by database)
                # - created_at, updated_at: excluded if None (so database can set defaults)
                filtered_data = {k: v for k, v in data.items() if v is not None and k != "id"}

                if not filtered_data:
                    raise ValueError(f"No valid data to insert into {self.table_name}")
