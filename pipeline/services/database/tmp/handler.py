...
class DatabaseHandler:
    ...
    def create_process_data(self, config, raw_groups=None, overwrite: bool = False) -> Optional[int]:
        """
        ...
        try:
            ...
            existing_process_status_id = self.pipeline_db._find_existing_pipeline_record(
                run_date=run_date,
                data_type=pipe_type,
                unit=unit_name,
                obj=obj,
            ...
            if existing_process_status_id:
                self._log_info(f"Found existing pipeline db record (PID: {existing_process_status_id})")
                if overwrite:
                    # Use cascade delete to properly handle foreign key constraints
                    try:
                        deleted = self.pipeline_db.delete_pipeline_cascade(existing_process_status_id)
                        if not deleted:
                            self._log_error(
                                f"Failed to delete existing pipeline db record (PID: {existing_process_status_id})"
                            )
                            raise ValueError(f"Failed to delete existing pipeline record for overwrite")
                        self._log_info(
                            f"Deleted existing pipeline db record (PID: {existing_process_status_id}) for overwrite"
                        )
                    except Exception as e:
                        self._log_error(
                            f"Error deleting existing pipeline db record (PID: {existing_process_status_id}): {e}"
                        )
                        raise
                else:
                    self.process_status_id = existing_process_status_id
                    config.process_id = existing_process_status_id
                    self._log_info(f"Using existing pipeline db record (PID: {existing_process_status_id})")
            ...
            if pipe_type == "masterframe":
                ...
                self.process_status_id = self.pipeline_db.create_process_data(pipeline_data)
                self._log_info(f"Created pipeline record with PID: {self.process_status_id}")
    ...
    def update_progress(self, progress: int, status: str = None) -> bool:
        ...
        if self.is_too and self.too_db is not None:
            try:
                ...
                return False
        elif self.process_status_id is None or self.pipeline_db is None:
            return False

        try:
            if status:
                self.pipeline_db.update_pipeline_data(self.process_status_id, progress=progress, status=status)
            else:
                self.pipeline_db.update_pipeline_data(self.process_status_id, progress=progress)
            return True
        except Exception as e:
    ...
    def add_warning(self, count: int = 1) -> bool:
        ...
        if self.process_status_id is None or self.pipeline_db is None:
            return False

        try:
            return self.pipeline_db.add_warning(self.process_status_id, count)
        except Exception as e:
    ...
    def add_error(self, count: int = 1) -> bool:
        ...
        try:
            return self.pipeline_db.add_error(self.process_status_id, count)
        except Exception as e:
    ...
    def reset_errors_warnings(self) -> bool:
        ...
        try:
            return self.pipeline_db.update_pipeline_data(self.process_status_id, errors=0, warnings=0)
        except Exception as e:
            self._log_warning(f"Failed to reset errors and warnings: {e}")
            return False

    def update_errors_warnings_from_log(self, config_file: str = None, process_status_id: int = None) -> bool:
        """
        ...
        Args:
            ...
            process_status_id: Pipeline ID (if provided, config_file will be fetched from database)
        ...
        try:
            # Get config_file from database if process_status_id is provided
            if process_status_id is not None:
                pipeline_data = self.pipeline_db.read_pipeline_data(process_status_id=process_status_id)
                if not pipeline_data or not pipeline_data.config_file:
                    self._log_warning(f"No config_file found for process_status_id {process_status_id}")
                    return False
                config_file = pipeline_data.config_file
                target_process_status_id = process_status_id
            elif config_file:
                # Find process_status_id from config_file
                ...
                if not pipeline_data:
                    ...
                    return False
                target_process_status_id = pipeline_data.id
            else:
                # Use self.process_status_id if available
                if self.process_status_id is None:
                    self._log_warning("No config_file or process_status_id provided")
                    return False
                pipeline_data = self.pipeline_db.read_pipeline_data(process_status_id=self.process_status_id)
                if not pipeline_data or not pipeline_data.config_file:
                    self._log_warning(f"No config_file found for process_status_id {self.process_status_id}")
                    return False
                config_file = pipeline_data.config_file
                target_process_status_id = self.process_status_id
            ...
            return self.pipeline_db.update_pipeline_data(
                target_process_status_id, errors=error_count, warnings=warning_count
            )
    ...
    def create_qa_data(
        ...
        # Skip QA data creation if process_status_id is not set
        if self.process_status_id is None:
            self._log_warning(f"Skipping QA data creation for {dtype}: process_status_id is not set")
            return
        ...
        if qa_data.unit is None and self.process_status_id is not None:
            pipeline_record = self.pipeline_db.read_pipeline_data(process_status_id=self.process_status_id)
    ...
    def update_qa_data(self, dtype: str, current_group: int = None, obj: str = None, **kwargs) -> bool:
        ...
        Returns:
            True if successful, False otherwise
        """
        if self.process_status_id is None or self.qa_db is None:
            return False

        try:
            # Find the QA record by process_status_id and dtype
            if dtype == "science":
                qa_type = f"{dtype}_{obj}"
                qa_data = self.qa_db.read_qa_data(process_status_id=self.process_status_id, qa_type=qa_type)
            elif dtype in ["bias", "dark", "flat"]:
    ...
    @property
    def has_process_status_id(self) -> bool:
        """Check if pipeline record exists"""
        return self.process_status_id is not None

    # ==================== COMBINED OPERATIONS ====================

    def get_pipeline_with_qa(self, process_status_id: int) -> Optional[Dict[str, Any]]:
        """Get pipeline data with associated QA data"""
        try:
            # Get pipeline data
            pipeline_record = self.pipeline_db.read_pipeline_data(process_status_id=process_status_id)
            if not pipeline_record:
                return None

            # Get associated QA data
            qa_data = self.qa_db.read_qa_data(process_status_id=process_status_id)
            if isinstance(qa_data, list):
                qa_list = qa_data
            else:
    ...
    def delete_pipeline_cascade(self, process_status_id: int) -> bool:
        ...
        try:
            ...
            return self.pipeline_db.delete_pipeline_cascade(process_status_id)
