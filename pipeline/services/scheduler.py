    ...
    def _mark_done_db(self, index, return_code=True, timeout=None):
        ...
        with self._db_connection(timeout=timeout) as conn:
            ...
            if return_code==SUCCESS_RETURN_CODE:
                ...
                cursor.execute(
                    'UPDATE scheduler SET status = ?, pid = 0, dispatch = NULL, process_end = ? WHERE "index" = ?',
                ...
                cursor.execute(
                    'UPDATE scheduler SET status = ?, readiness = ?, is_ready = ?, pid = 0, dispatch = NULL, process_end = ? WHERE "index" = ?',
