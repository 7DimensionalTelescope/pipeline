...
class PathHandler(AutoMkdirMixin, AutoCollapseMixin):
    ...
    def define_file_dependent_paths(self):
        ...
        if self._daily_coadd_dir:
            self.daily_coadd_dir = self._daily_coadd_dir
        if self._subtracted_dir:  # unused
            self.subtracted_dir = self._subtracted_dir
        if self._coadd_dir:
            self.coadd_dir = self._coadd_dir
    ...
    @classmethod
    def take_raw_inventory(
        cls,
        files: list[str],
        lone_calib: bool = True,
        is_too: bool = False,
        is_pipeline: bool = False,
    ):
    ...
    def build_preproc_input(
        ...
        for (bias, dark, flat), sci in zip(associated_calib, sci_files):
            mbias, mdark, mflat = cls(sci[0], is_pipeline=is_pipeline).preprocess.masterframe  # [0]: trust the grouping
        ...
        for _key, entry in sorted(
            ...
            for sci_group in entry["sci"]:
                ...
                sci_dict[key] = (
                    sci_group,
                    atleast_1d(cls(sci_group, is_pipeline=is_pipeline, is_too=is_too).conjugate),
                )
            ...
            mbias, mdark, mflat = cls(sample_file, is_pipeline=is_pipeline).preprocess.masterframe  # trust the grouping
        ...
        if allow_unassociated_calib and unassociated_calib and any(l for l in unassociated_calib):
            unassociated_raw_bias_groups, unassociated_raw_dark_groups, unassociated_raw_flat_groups = unassociated_calib  # fmt: skip
            for unassociated_raw_bias_group in unassociated_raw_bias_groups:
                if len(unassociated_raw_bias_group) < const.NUM_MIN_CALIB:
                    continue

                mbias = cls(unassociated_raw_bias_group, is_pipeline=is_pipeline).preprocess.masterframe
                # preprocess.mbias works too
                result.append([[sorted(unassociated_raw_bias_group), [], []], [mbias, "", ""], dict()])

            for unassociated_raw_dark_group in unassociated_raw_dark_groups:
                if len(unassociated_raw_dark_group) < const.NUM_MIN_CALIB:
                    continue

                mdark = cls(unassociated_raw_dark_group, is_pipeline=is_pipeline).preprocess.masterframe
                mbias = cls(
                    unassociated_raw_dark_group, is_pipeline=is_pipeline
            ...
            for unassociated_raw_flat_group in unassociated_raw_flat_groups:
                if len(unassociated_raw_flat_group) < const.NUM_MIN_CALIB:
                    continue

                mflat = cls(unassociated_raw_flat_group, is_pipeline=is_pipeline).preprocess.masterframe
                self = cls(unassociated_raw_flat_group, is_pipeline=is_pipeline)
...
class PathImcoadd(AutoMkdirMixin, AutoCollapseMixin):
    def __init__(self, parent: PathHandler, config=None):
        self._parent = parent

        # Apply config overrides if provided
        if config and hasattr(config, "path"):
            for key, val in config.imcoadd.path.items():
                setattr(self, key, val)
    ...
    @property
    def tmp_dir(self):
        return os.path.join(self._parent.factory_dir, IMCOADD_TMP_DIRNAME)

    @property
    def figure_dir(self):
        return os.path.join(self._parent.figure_dir, IMCOADD_TMP_DIRNAME)

    @property
    def coadd_image_basename(self):
        ...
        exptime_str = format_exptime(total_exptime, type="coadd")
...
class PathImsubtract(AutoMkdirMixin, AutoCollapseMixin):
    ...
    @property
    def tmp_dir(self):
        return os.path.join(self._parent.factory_dir, IMSUBTRACT_TMP_DIRNAME)

    @property
    def figure_dir(self):
        return os.path.join(self._parent.figure_dir, IMSUBTRACT_TMP_DIRNAME)
