...
warnings.filterwarnings(
    "ignore", message="Warning: 'partition' will ignore the 'mask' of the MaskedColumn."
)
...
path_calibration_field = (
    "/large_data/Calibration/7DT-Calibration/output/Calibration_Tile"
)
...
for key in list(aperture_dict.keys()):
    add_aperture_dict[key.replace("MAG_", "")] = (
        round(aperture_dict[key][0], 3),
        aperture_dict[key][1],
    )
# 	MAG KEY
...
skymed, skysig = float(line[0].split("Background:")[1].split("RMS:")[0]), float(
    line[0].split("RMS:")[1].split("/")[0]
)
