...
def iter_processed(
    ...
    for date_dir in sorted(glob(os.path.join(base, date_pattern))):
        ...
        for target_dir in sorted(glob(os.path.join(date_dir, obj_pattern))):
            ...
            # 3) filters: /.../<date>/<target>/*
            for filter_dir in sorted(glob(os.path.join(target_dir, filter_pattern))):
                if not os.path.isdir(filter_dir):
                    continue

                singles_dir = os.path.join(filter_dir, type_pattern)
                if not os.path.isdir(singles_dir):
                    continue

                # 4) files in singles/: /.../singles/*s.fits
                for path in sorted(glob(os.path.join(singles_dir, filename_pattern))):
                    yield path
