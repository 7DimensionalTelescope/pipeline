...
def calc_weight_with_cpu(images, d_m_file, f_m_file, sig_z_file, sig_f_file, weight=True, **kwargs):
    ...
    stacked = np.stack(output_data)  # shape (N, H, W)
    N, H, W = stacked.shape

    sig_r2 = np.empty((H, W), dtype=np.float32)
    sig_rp2 = np.empty((H, W), dtype=np.float32)

    # process each image
    for idx in range(N):
        r_p = stacked[idx]
        # compute sig_r_squared
        compute_sig_r(r_p, f_m, d_m, egain, sig_z, sig_r2)
        # compute sig_rp_squared
        compute_sig_rp(sig_r2, sig_zm, sig_dm_sq, f_m, r_p, sig_fm, sig_rp2)

        if weight:
            stacked[idx] = 1.0 / (sig_rp2 + sig_b**2)
        else:
            stacked[idx] = np.sqrt(sig_rp2 + sig_b**2)
...
def compute_sig_r(sci, flat, dark, gain, sig_z, out):
    ...
    for i in prange(n0):
        for j in range(n1):
            corrected = sci[i, j] * flat[i, j] + dark[i, j]
            clipped = corrected if corrected > 0.0 else 0.0
...
def compute_sig_rp(sig_r_squared, sig_zm, sig_dm_sq, f_m, r_p, sig_fm, out):
    ...
    """
    n0, n1 = sig_r_squared.shape
    for i in prange(n0):
        for j in range(n1):
            fm2 = f_m[i, j] * f_m[i, j]
            term1 = (sig_r_squared[i, j] 
                     + sig_zm[i, j] * sig_zm[i, j] 
                     + sig_dm_sq[i, j]) / fm2
            term2 = (r_p[i, j] * r_p[i, j]) * (sig_fm[i, j] * sig_fm[i, j]) / fm2
            out[i, j] = term1 + term2
